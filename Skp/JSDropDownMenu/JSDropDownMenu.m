...
- (void)setDataSource:(id<JSDropDownMenuDataSource>)dataSource {
    ...
    for (int i = 0; i < _numOfMenu; i++) {
        ...
        //separator
         if (i != _numOfMenu - 1) {
             CGPoint separatorPosition = CGPointMake((i + 1) * separatorLineInterval, self.frame.size.height/2.0);
...
//    [path addLineToPoint:CGPointMake(160, self.frame.size.height)];
    [path moveToPoint:CGPointMake(160,self.frame.size.height *1/4.0f)];
    [path addLineToPoint:CGPointMake(160, self.frame.size.height *3/4.0f)];
    //end modify
